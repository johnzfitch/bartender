...
const CONFIG_TOML_PATH = `${CONFIG_DIR}/config.toml`
...
class ConfigService {
  ...
  /**
   * Migrate old config.json to config.toml on first run
   */
  private _migrateIfNeeded(): void {
    ...
    // Only migrate if TOML doesn't exist but JSON does
    if (tomlFile.query_exists(null) || !jsonFile.query_exists(null)) {
      return
    }

    console.log("[Config] Migrating config.json to config.toml...")
    ...
    try {
      ...
      // Strip comments for JSONC support (but not // inside strings)
      const stripped = this._stripJsonComments(raw)
      const oldConfig = JSON.parse(stripped)
      const newConfig = this._migrateOldConfig(oldConfig)

      // Write new TOML config
      this._writeTomlConfig(newConfig)

      // Rename old config to .json.bak
      const backupPath = `${CONFIG_JSON_PATH}.bak`
      jsonFile.move(
      ...
      console.log(`[Config] Migration complete. Old config backed up to ${backupPath}`)
    } catch (e) {
